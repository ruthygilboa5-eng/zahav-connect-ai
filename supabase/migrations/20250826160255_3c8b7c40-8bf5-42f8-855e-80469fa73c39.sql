...
DROP POLICY IF EXISTS "Main users can manage family roles" ON user_roles;
...
CREATE POLICY "Primary users can manage family roles" 
ON user_roles 
FOR ALL
TO authenticated
USING (has_role(auth.uid(), 'primary_user'::app_role) OR (user_id = auth.uid()));
